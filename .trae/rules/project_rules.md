...
> Full regression guard rules (R1-R157) are split by category in `.trae/rules/regression/`.
...
**Quick reference — all 141 guards by category:**
...
| UI 健壮性 | 9 | 界面不崩、有反馈、无泄漏（含 R131 error boundary error count, R132 filter/refresh wiring, R133 drag handlers non-empty） |
| 工程质量 | 22 | 依赖合规、构建安全、测试可靠（含 R107 upload size limit, R135 secureConfigRouteSchema, R136 network-monitor deferred side effects, R137 video-cache deferred beforeunload, R154 useAssetLoader Promise.all concurrency, R155 StoryProvider services useMemo, R156 useVideoTasksPage single-pass stats memo） |
