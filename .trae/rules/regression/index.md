...
| 三、错误处理 | R5, R6, R15, R17, R18, R44, R47, R50, R53, R56, R63, R86, R108, R129, R134, R136, R192, R195 | 18 | [error-handling.md](error-handling.md) |
...
| 五、工程质量 | R3, R26, R27, R28, R33, R39, R40, R41, R54, R55, R57, R58, R59, R60, R87, R88, R92, R107, R135, R146, R147, R154, R155, R156, R159, R162, R165, R166, R175, R176, R177, R178, R179, R180, R181, R182, R188, R189, R191, R193, R194 | 41 | [engineering.md](engineering.md) |
...
## R192-R195 新增规则速览（quality-gate + cost-tracking 设计评审产物）

> 以下规则为两份设计文档（`docs/DESIGN-QUALITY-GATE.md` / `docs/DESIGN-COST-TRACKING.md`）评审修订（v0.2）产物，与质检层 P0 骨架（`src/shared-logic/quality-gate/`）及成本追踪 P0 同批落地。

| 规则 | 主题 | 分类 | 测试文件 |
|------|------|------|---------|
| R192 | quality-gate 编排器（QualityGateRunner.run）绝不 throw，异常降级为 warn 空报告 | 错误处理 | `src/shared-logic/quality-gate/__tests__/runner.test.ts` |
| R193 | quality-gate 降级链只允许"同类能力降档"，禁止跨语义替换；报告必须标注 standardsUsed | 工程质量 | `src/shared-logic/quality-gate/__tests__/runner.test.ts` |
| R194 | quality-gate 阈值解析必须 per-model > per-provider > default，且 clamp 到 [0,1] | 工程质量 | `src/shared-logic/quality-gate/__tests__/thresholds.test.ts` |
| R195 | usage 记录链路（recordUsage）失败必须静默降级，绝不阻塞生成主流程 | 错误处理 | `src/modules/cost-tracking/__tests__/usage-tracker.test.ts` |
