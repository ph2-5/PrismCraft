...
**shared-logic 层**是纯逻辑层，零外部依赖。它包含提示词引擎（prompt-engine、prompt-service）、引用引擎（reference-engine、consistency-check、reference-check、visual-consistency-check）、视频追踪（video-task-params、video-tracker、video-recovery）和故事服务（story-service、storyboard-generation）。shared-logic 只允许相对导入，不能导入 `@/`、`@shared/`、`@domain/` 或任何项目层。渲染进程使用 `@/shared-logic/*` 路径别名，主进程使用 `@shared-logic/*`。
...
| 模块 | 代码行数 | 子域数 | 核心职责 |
|------|---------|--------|---------|
| story | 19878 | 5 | 故事创作与分镜管理、提示词生成与编排、批量视频生成 |
| video | 12101 | 4 | 视频任务管理、缓存、恢复、编解码检测、帧提取 |
| shot | 5400 | 7 | 分镜系统：一致性检查、元素绑定、特征提取、镜头指令 |
| prompt | 4482 | 8 | 提示词生成与管理：角色/场景/分镜/视频提示词构建 |
| asset | 3404 | 5 | 资产库管理：媒体资产、导入导出、项目备份与恢复 |
| sync | 2460 | 2 | 多设备数据同步：变更追踪、向量时钟、冲突检测与解决 |
| character | 2872 | 3 | 角色管理：角色 CRUD、服装管理、角色图片生成 |
| scene | 1855 | 3 | 场景管理：场景 CRUD、图片生成 |
| persistence | 982 | 0 | 自动保存、持久化守护、事务性删除 |
| agent | 35417 | 5 | AI 助手模块（Agent Loop、Tool Registry、20 域 154 工具、记忆系统、流式输出） |

### 4.2 各模块详细说明

#### storyboard 模块

**职责**：分镜管理、提示词生成与编排、批量视频生成。这是项目中最大的模块，承载了从分镜编排到视频生成的核心工作流。（故事创作流水线已拆分至 novel 模块，详见 [novel-pipeline-guide.md](novel-pipeline-guide.md)）
...
| prompt-editor | AI 辅助提示词生成、编辑、预览、浮动助手 |
...
5. **CQRS 模式**：视频任务管理遵循 CQRS（Command Query Responsibility Segregation）模式，将 Zustand Store 拆分为四层：`useVideoTaskState`（纯状态，无副作用）、`useVideoTaskQueries`（只读派生数据）、`useVideoTaskCommands`（写操作）、`useVideoTaskPolling`（轮询）。`useVideoTaskManager` 作为向后兼容的组合接口。

6. **stableActions 模式**：`useVideoTaskManager` 内部通过 `useMemo` 将所有 action 方法（addTask、createTask、pollTask 等）缓存为稳定引用。这些方法来自 `store.getState()`，其引用永不变化，因此 `stableActions` 对象的 `useMemo` 依赖为常量 `[store]`。这避免了 `allTasks` 变化时 action 方法引用也变化，导致消费方（如 StoryProvider）的 `useMemo` 被不必要地触发。

7. **setAllTasks 不自动触发 sync/polling**：`setAllTasks` 仅更新 Zustand 状态，不再自动调用 `scheduleSync()` 和 `checkAndStartOrStopPolling()`。所有写操作在调用 `setAllTasks` 后显式调用 `scheduleSync()` + `checkAndStartOrStopPolling()`。轮询引擎在批量更新后统一触发一次 sync/polling 检查，使用动态 `import("./sync-engine")` 避免循环依赖。

8. **useStableCompletedUrls**：`useStoryVideo` 中的 `completedTaskUrls` Map 通过 shallow 比较确保只有内容真正变化时才创建新引用，防止轮询更新（tasks 数组引用变化但内容不变）触发下游 `useEffect`（如 `useStoryPersistence`）。

9. **API 连接测试错误建议**：`testConnection()` 失败时，UI 根据 HTTP 状态码显示具体错误原因和可操作建议：401/403 → 检查 API Key，404 → 检查 Base URL，0/5xx → 检查网络。

10. **mapUserFacingError**：用户可见错误消息通过 `mapUserFacingError()` 映射为中文友好消息，禁止直接展示 `e.message` 或技术术语。**使用边界**：`catch(err)` 中的原始异常必须用 `mapUserFacingError(err)`，但 `result.error`（Result 类型的已处理字符串）应直接展示 `result.error || t("...")`，不应再包装 `mapUserFacingError`（否则会丢失具体信息变成通用"操作失败"）。
...
1. **7 个子域的细粒度划分**：shot 模块虽然只有 5400 行代码，却划分了 7 个子域。这是因为分镜系统涉及多个独立关注点——一致性检查需要理解视觉特征，元素绑定需要管理跨实体关系，镜头指令需要将自然语言转换为结构化参数。这些关注点各自独立演化，混合在一起会导致职责纠缠。

2. **元素管理器和引用引擎作为 DI 懒加载**：`elementManager` 和 `referenceEngine` 是 DI 容器中的懒加载 Token。它们通过 `await import()` 动态加载，原因是这两个模块与 storyboard 模块存在双向依赖——storyboard 需要 shot 的元素绑定能力，shot 的引用引擎需要 storyboard 的角色/场景数据。懒加载打破了初始化阶段的循环依赖。
...
3. **`getDeviceId()` 异步化与双轨通信**：`changelog.ts` 中的 `getDeviceId(): Promise<string>` 是异步函数，采用与 `@/shared/file-http` 一致的双轨通信策略——优先通过 HTTP API（`/api/config/get`）读取 `sync_device_id`，失败回退到 IPC（`electronAPI.getConfig`），并使用内存缓存 `_cachedDeviceId` 避免重复往返。设备 ID 持久化在 `localStorage`（`DEVICE_ID_STORAGE_KEY`），首次缺失时由主进程生成并写回。
...
2. **与 character 模块的对称设计**：scene 模块的结构与 character 模块几乎完全对称——相同的 services + hooks + presentation 子域划分、相同的 Result 模式、相同的 React Query Hooks 封装。这种对称性降低了认知负担，开发者理解一个模块后可以快速理解另一个。
...
### 5.2 Token 分类（6 类 46 个）

| 类别 | 数量 | 说明 | 示例 |
|------|------|------|------|
| A. Domain Port 实现 | 13 | Port 接口的具体实现 | videoProvider, characterStorage, imageProvider, textProvider, fileUploader, syncStorage, videoTaskStorage, sceneStorage, storyStorage, subShotStorage, generationAssetStorage, embeddingProvider, audioProvider |
| B. 有状态服务 | 6 | 单例，需测试替换 | eventBus, apiClient, imageApi, videoApi, textApi, preferencesStorage |
| C. Storage 实例 | 18 | 有状态存储，模块无法直接导入 infrastructure/storage | versionStorage, elementStorage, videoCacheStorage, imageCacheStorage（暴露 `flushPendingAccessUpdates()` 用于延迟元数据更新）, collectionStorage, storyboardStorage, importExportStorage, templateStorage, autoSaveStorage, errorLogStorage, sessionStorage, novelProjectStorage, propStorage, characterVariantStorage, sceneVariantStorage, timelineStorage, plotNodeStorage, storyTemplateStorage |
| D. Repository 实例 | 1 | Drizzle ORM | mediaAssetRepository |
| E. 懒加载模块 | 4 | 动态 `import()` 加载，避免循环依赖 | fileStorage, elementManager, referenceEngine, syncEngine |
| F. Agent 服务 | 4 | Agent 模块动态导入（E 类特化，避免 infrastructure 静态依赖 modules） | agentConversationManager, agentToolRegistry, agentToolExecutor, agentMemoryService |
...
**E 类 Token 的懒加载原因**：`elementManager` 和 `referenceEngine` 位于 `@/modules/shot/` 下，但被其他模块通过 DI 容器引用。`syncEngine` 位于 `@/modules/sync/` 下，`fileStorage` 通过工厂动态选择实现（LocalFileStorage/S3FileStorage）。如果静态导入，会在模块初始化阶段形成循环依赖。懒加载将依赖解析推迟到首次使用时，此时所有模块已完成初始化。F 类 Token 是 E 类的特化，专门用于 Agent 模块的动态导入，避免 infrastructure 静态依赖 modules。完整 Token 清单详见 [di-tokens.md](di-tokens.md)。
...
| 应该注册 | 不应该注册 |
|---------|-----------|
| Port 接口实现（需替换测试替身） | 纯函数（如 `resolveImageUrl`、`mapUserFacingError`） |
...
| `@/shared/model-capabilities` | `getModelCapabilities`, `getVideoGenerationStrategy`, `ImageSizePurpose` | `infrastructure/ai-providers/model-capabilities` | AI 模型能力查询、视频生成策略（参考图传递方式、prompt语言） |
| `@/shared/user-facing-error` | `mapUserFacingError` | `shared/utils/user-facing-error` (uses classifyError from domain) | 用户友好错误消息映射 |
| `@/shared/file-http` | `writeFile`, `readFile`, `getFileInfo`, `getCacheDirectory`, `getDiskSpace`, `fileExists`, `deleteFile` | `shared/file-http`（HTTP 优先 + IPC 回退，非 infrastructure 代理） | 统一文件操作通信层 |

### 5.5 统一文件操作通信层（`@/shared/file-http`）

`src/shared/file-http/` 是文件操作的统一通信层，采用 **HTTP 优先 + IPC 回退** 的双轨设计。模块在首次调用时通过 `/api/health` 探测 HTTP API Server 可用性并缓存结果（`_httpAvailable`），后续调用优先走 `http://localhost:${API_SERVER_PORT}/api/file/*` 端点，HTTP 不可用或调用失败时自动回退到 `window.electronAPI` 的 IPC 接口。

**公开函数（7 个）**：`writeFile`、`readFile`、`getFileInfo`、`getCacheDirectory`、`getDiskSpace`、`fileExists`、`deleteFile`（测试用 `_resetHttpCache` 重置探测缓存）。

**设计动机**：视频/图片缓存服务（`video-cache.ts`、`image-cache.ts`）原直接依赖 IPC，迁移到 `@/shared/file-http` 后获得 HTTP 通道的批量传输能力与统一错误处理，同时保留 IPC 回退保证向后兼容（HTTP API Server 未启动时仍可工作）。HTTP 调用失败时会将 `_httpAvailable` 置为 `false` 触发后续走 IPC，避免持续探测失败通道。
...
**Schema 版本控制**：`CURRENT_SCHEMA_VERSION=4`，迁移在 `db.transaction()` 中执行确保原子性。v3 迁移添加了 `local_video_path` 等列，支持视频本地缓存路径。v4 迁移为 `collection_assets` 关联表补齐 `created_at`、`updated_at` 列，使收藏集资产记录具备时间戳追踪能力。
...
electron/src/api/
  ...
  routes.ts       → 路由注册表（合并所有路由组）
  route-groups/   → 路由处理器分组（7 个路由组）
    core-routes.ts       → 配置、上传、导出、测试连接、同步路由
    db-routes.ts         → 数据库查询、运行、批量插入、事务路由
    file-routes.ts       → 文件读写、缓存目录、磁盘空间、文件信息路由
    generation-routes.ts → 图片/视频/文本生成、故事生成路由
    plugin-routes.ts     → 插件管理路由（列表、添加、删除、重载等）
    shot-routes.ts       → 分镜引用、一致性检查、视觉一致性路由
    storyboard-routes.ts → 分镜板生成、视频恢复、批量保存路由
...
- 请求体大小限制：100MB（`file/write` 路由的 `MAX_WRITE_SIZE = 100 * 1024 * 1024`）
...
14 个内置 Provider 通过声明式插件注册：

| Provider | ID | 视频 | 图片 | 文本 | 视觉 |
|----------|-----|------|------|------|------|
| 火山引擎 (Doubao) | volcengine | 是 | 是 | — | — |
| 可灵AI (Kling) | kuaishou | 是 | 是 | — | — |
| 智谱AI (GLM) | zhipu | 是 | 是 | — | 是 |
| Pixverse | pixverse | 是 | 是 | — | — |
| Seedance | seedance | 是 | 是 | — | — |
| Google (Veo/Gemini) | google | 是 | 是 | 是 | 是 |
| OpenAI (Sora/DALL-E) | openai-sora | 是 | 是 | — | — |
| MiniMax (Hailuo) | minimax | 是 | 是 | — | — |
| Anthropic (Claude) | anthropic | — | — | 是 | 是 |
| OpenAI Compatible | openai-compatible | 是 | 是 | 是 | 是 |
| Runway | runway | 是 | — | — | — |
| Luma Dream Machine | luma | 是 | — | — | — |
| Pika | pika | 是 | — | — | — |

另有 10 个声明式 JSON Provider（由 OpenAI Compatible 插件或 UserPluginAdapter 处理）：

| Provider | ID | 说明 |
|----------|-----|------|
| OpenAI | openai | GPT/DALL-E 系列 |
...
| BytePlus | byteplus | 字节跳动云 |
| Fireworks AI | fireworks | Fireworks 推理平台 |
| Amazon Bedrock | bedrock | AWS Bedrock |
| Ollama (本地) | ollama | 本地模型推理 |
| Pollinations (免费) | pollinations | 免费图片生成 |
...
**参考图策略系统**：不同 AI 模型对角色参考图和场景参考图的支持方式不同。`getVideoGenerationStrategy(modelId)` 返回 `VideoGenerationStrategy`，包含 `useCharacterRef`/`useSceneRef`（是否通过视频 API 原生传递）、`characterRefMode`/`sceneRefMode`（传递方式：`native_field` 原生字段、`bake_into_first` 融入首帧、`ref_field` 参考图字段、`both` 两者兼有）、`promptLanguage`（prompt 语言偏好）。**bake_into_first 模式**：Seedance pro 系列模型使用此模式，因为其视频 API 的三种模式（首帧/首尾帧/参考图）互斥，pro 模型不支持在首尾帧模式中同时传递参考图。参考图信息通过"融入首帧"架构传递——首帧生成时将参考图 URL 传给图片生成模型（如 Seedream 的 `ref_image` 参数），让首帧图片本身就包含角色/场景特征。视频生成阶段只传首帧/尾帧，不传参考图。**ref_field 模式**：Seedance lite-i2v 使用此模式，通过 content 数组中 `role: "reference_image"` 传递参考图（1-4张）。**能力声明修正**：Kling V1 不支持原生角色参考（仅 V2+ 支持 `subject_reference`）；Google Veo 不支持尾帧和参考图；CogVideoX 不支持尾帧；MiniMax 仅 S2V-01 模型支持 `subject_image_url`。
...
**SSRF 防护**：`ssrf-guard.ts` 模块对非 loopback 主机启用 SSRF 校验（R105），拦截云元数据端点（169.254.169.254）并做 DNS rebinding 检查。loopback 主机（`127.0.0.1`、`localhost`、`::1`）受信任直接放行——支持用户配置的内网自部署 AI 服务（如 Ollama、vLLM）。用户配置的非 loopback 主机仍走 `ssrfGuard.validate` 做 DNS 解析与私有 IP 检查；非用户配置的 URL 强制走完整 SSRF 校验。
...
**35 个结构化错误码**：从 `DATABASE_ERROR` 到 `LOG`，每个错误码包含 code（字符串标识）、domain（所属域）、description（中文描述）、retryable（是否可重试）。`isRetryable(code)` 函数查询错误码的可重试属性，指导重试决策。
...
### 9.3 152 条回归防护（R1–R191，编号非连续，8 大类）
...
152 条回归防护来自多次定向 Bug 审计，审计遵循"使用驱动发现 + 结构化验证固化"方法论。完整操作手册见 `docs/bug-audit-methodology.md`，此处记录核心框架。
...
- `npm install` 安装依赖
- `npm run dev` 启动 Vite 开发服务器（仅渲染进程，无 Electron）
...
**覆盖率阈值**：Branches 80%、Functions 80%、Lines 80%、Statements 80%，perFile 强制执行。覆盖范围包括 domain schemas、services、infrastructure core、shared utils。
...
2. **Electron TypeScript 编译**：`tsc -p electron/tsconfig.json` 编译主进程代码
3. **文件复制**：编译产物、插件文档复制到输出目录
4. **electron-builder 打包**：`out/` 打包为 asar，better-sqlite3 原生模块通过 `asarUnpack` 解包
...
- **storyboard 模块体量大**：19878 行，包含 5 个子域（beat-editor、generation、planning、template、prompt-editor），功能内聚性较强，暂不拆分。故事创作流水线已拆分至 novel 模块，详见 [novel-pipeline-guide.md](novel-pipeline-guide.md)。
- **大文件拆分进展**：api-gateway.ts（991→360+utils+image）、user-plugin-loader.ts（972→286+adapter）、templates.ts（996→191+provider-templates-data）、routes.ts（921→15+5 route-groups）已完成拆分，当前无超过 500 行的 Electron 端文件
...
- **UI 层国际化完成**：2026 键的 messages.ts 已覆盖核心 UI 文本，app/pages、modules/presentation、shared/presentation、modules/hooks 中用户可见文本已全部迁移为 t() 调用。AI 提示词模板和日志文本按规则不迁移
