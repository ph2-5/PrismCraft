...
项目包含 9 个业务模块（story、video、shot、prompt、asset、sync、character、scene、persistence），每个模块有独立的演化节奏。story 模块 16502 行代码、5 个子域，而 persistence 模块仅 434 行、2 个子域。DDD 的子域划分确保大模块内部不会因无序依赖而腐化，跨模块的依赖必须通过桶文件（barrel file）的公共 API，深层路径导入被 ESLint 规则和架构扫描脚本双重拦截。这种约束在 9 个模块、43 个子域的规模下是必要的——没有它，模块边界会在数周内模糊消失。
...
| 模块 | 代码行数 | 子域数 | 核心职责 |
|------|---------|--------|---------|
| story | 16502 | 5 | 故事创作与分镜管理、提示词生成与编排、批量视频生成 |
...
- **story 模块体量大**：16502 行（占模块总量 35.5%），包含 5 个子域（beat-editor、generation、planning、template、prompt-editor），功能内聚性较强，暂不拆分
