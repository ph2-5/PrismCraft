...
- **WASM 冗余包**：`@emnapi/core`、`@emnapi/runtime` 等 better-sqlite3 的 optional deps 存在于 node_modules 中，无法修剪，无害
- **测试类型排除**：`tsconfig.json` 排除所有测试文件，Vitest 独立处理测试类型，MVP 阶段可接受
- **硬编码中文字符串**：约 19000+ 处中文字符串分布在 100 个文件中，作为中文桌面应用，完整 i18n ROI 低；如需多语言支持，应提取共享消息常量
