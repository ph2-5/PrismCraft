...
项目包含 9 个业务模块（story、video、shot、prompt、asset、sync、character、scene、persistence），每个模块有独立的演化节奏。story 模块 19878 行代码、5 个子域，而 persistence 模块仅 982 行。DDD 的子域划分确保大模块内部不会因无序依赖而腐化，跨模块的依赖必须通过桶文件（barrel file）的公共 API，深层路径导入被 ESLint 规则和架构扫描脚本双重拦截。这种约束在多模块、多子域的规模下是必要的——没有它，模块边界会在数周内模糊消失。
...
另有 9 个 Provider 通过 JSON 配置声明（由 OpenAI Compatible 插件或 UserPluginAdapter 处理）：
...
104 条回归防护来自四次定向 Bug 审计，审计遵循"使用驱动发现 + 结构化验证固化"方法论。完整操作手册见 `docs/bug-audit-methodology.md`，此处记录核心框架。
