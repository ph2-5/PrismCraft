...
项目包含 9 个业务模块（story、video、shot、prompt、asset、sync、character、scene、persistence），每个模块有独立的演化节奏。story 模块 10574 行代码、5 个子域，而 persistence 模块仅 259 行。DDD 的子域划分确保大模块内部不会因无序依赖而腐化，跨模块的依赖必须通过桶文件（barrel file）的公共 API，深层路径导入被 ESLint 规则和架构扫描脚本双重拦截。这种约束在多模块、多子域的规模下是必要的——没有它，模块边界会在数周内模糊消失。
...
| 模块 | 代码行数 | 子域数 | 核心职责 |
|------|---------|--------|---------|
| story | 10574 | 5 | 故事创作与分镜管理、提示词生成与编排、批量视频生成 |
| video | 6624 | 4 | 视频任务管理、缓存、恢复、编解码检测、帧提取 |
| shot | 2856 | 7 | 分镜系统：一致性检查、元素绑定、特征提取、镜头指令 |
| prompt | 1813 | 8 | 提示词生成与管理：角色/场景/分镜/视频提示词构建 |
| asset | 2278 | 5 | 资产库管理：媒体资产、导入导出、项目备份与恢复 |
| sync | 2156 | 2 | 多设备数据同步：变更追踪、向量时钟、冲突检测与解决 |
| character | 977 | 3 | 角色管理：角色 CRUD、服装管理、角色图片生成 |
| scene | 530 | 3 | 场景管理：场景 CRUD、图片生成 |
| persistence | 259 | 0 | 自动保存、持久化守护、事务性删除 |
...
- **story 模块体量大**：10574 行（占模块总量 37.7%），包含 5 个子域（beat-editor、generation、planning、template、prompt-editor），功能内聚性较强，暂不拆分
