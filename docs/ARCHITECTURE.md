...
项目包含 11 个业务模块（story、video、shot、prompt、asset、sync、character、scene、persistence、agent），每个模块有独立的演化节奏。story 模块 19878 行代码、5 个子域，而 persistence 模块仅 982 行。DDD 的子域划分确保大模块内部不会因无序依赖而腐化，跨模块的依赖必须通过桶文件（barrel file）的公共 API，深层路径导入被 ESLint 规则和架构扫描脚本双重拦截。这种约束在多模块、多子域的规模下是必要的——没有它，模块边界会在数周内模糊消失。
...
modules/         → 11 个业务子域模块，每个有 hooks/services/presentation
...
**modules 层**包含 11 个业务模块，每个模块遵循 `index.ts`（桶文件）+ `MODULE.md`（契约）+ 子域目录的结构。模块只能导入 `@/domain/*`、`@/shared/*` 和 `@/infrastructure/di`（通过 DI 容器获取基础设施实例）。
...
- **story 模块体量大**：19878 行（占模块总量 32.5%），包含 5 个子域（beat-editor、generation、planning、template、prompt-editor），功能内聚性较强，暂不拆分。故事创作流水线设计详见 [story-pipeline-design.md](story-pipeline-design.md)。
