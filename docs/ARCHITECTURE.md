...
### 为什么选择 Electron + Next.js

桌面端需要本地文件系统访问、SQLite 数据库、原生窗口管理、系统托盘等能力，浏览器 Web 应用无法满足。Electron 提供完整的 Node.js 运行时和原生 API 访问。Next.js 则提供成熟的路由系统、组件生态和开发体验。两者结合的关键决策是 `output: "export"`——Electron 渲染进程通过静态文件服务器加载页面，不需要 Next.js 服务端运行时，从而避免了在 Electron 中运行 Node.js 服务器的冗余开销。
...
- **Next.js 16**（`output: "export"`）：静态导出模式，不使用服务端功能
...
1. **临时移除 `src/app/api/`**：Next.js `output: "export"` 不允许存在 API 路由，构建前将整个 api 目录移出，构建后恢复
2. **Next.js static export**：`next build` 生成静态 HTML/JS/CSS 到 `out/`
3. **Electron TypeScript 编译**：`tsc -p electron/tsconfig.json` 编译主进程代码
4. **文件复制**：编译产物、插件文档复制到输出目录
5. **electron-builder 打包**：`out/` 打包为 asar，better-sqlite3 原生模块通过 `asarUnpack` 解包（原生模块无法在 asar 内加载）

为什么构建时要移除 api 目录而不是直接删除？因为开发模式下 `npm run dev` 仍然需要这些 API 路由进行调试。移除-恢复模式确保开发体验不受影响，同时满足生产构建的约束。
...
- **测试类型排除**：`tsconfig.json` 排除所有测试文件，已有 `tsconfig.test.json` 为测试提供独立类型检查，Vitest 运行时独立处理测试类型
- **硬编码中文字符串**：约 15734 处中文字符串分布在 100 个文件中，已有 `@/shared/constants/messages.ts` 消息常量基础设施，逐步提取中
- **better-sqlite3 版本锁定**：`12.10.0`（无 ^前缀），锁定原因是原生模块升级可能导致 Electron 重建失败，升级需手动验证兼容性
- **构建流程 API 路由 hack**：`build-electron.ps1` 构建时临时移除 `src/app/api/` 再恢复，根因是 Next.js `output: "export"` 不兼容 API Route；长期方案是将 API 路由迁移到 Electron 主进程 HTTP API Server
