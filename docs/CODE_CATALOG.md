...
AI Animation Studio 是一款 AI 驱动的动画制作工具，采用本地优先架构，支持从故事创作到视频生成的完整工作流。项目基于 Electron + Vite + React 技术栈，采用 DDD（领域驱动设计）分层架构，将代码组织为 domain → shared-logic → modules → shared → infrastructure → app 六层。
