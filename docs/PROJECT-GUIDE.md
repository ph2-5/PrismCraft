...
> 版本：0.9.0 | 许可证：MIT | 最后更新：2026-06-03
...
67 条回归防护规则（R1-R67）是从历史 Bug 中抽象出的检测规则，防止已知 Bug 模式重现。这些规则是**回归防护**，不是发现工具——下一次审计必须从零开始发现新问题。

规则按 6 大类组织：

| 类别 | 规则数 | 核心关注 |
|------|--------|---------|
| 数据一致性 | 14 | 数据不丢、不脏、不冲突 |
