...
> 版本：0.9.1 | 许可证：MIT | 最后更新：2026-06-06
...
AI Animation Studio 是一款**本地优先（local-first）、离线可用（offline-capable）**的 AI 动画制作桌面工具。它覆盖了从创意到成品的完整动画制作工作流：故事构思 → 角色设计 → 场景搭建 → 分镜编排 → AI 视频生成 → 导出成品。

与云端动画工具不同，AI Animation Studio 的核心理念是**数据主权**——所有用户数据（故事、角色、场景、视频任务、资产）都存储在本地 SQLite 数据库中，用户完全掌控自己的创作数据。AI 生成能力通过可插拔的 Provider 插件对接外部 API（火山引擎、快手、智谱、PixVerse、Seedance、Google、OpenAI Sora、MiniMax、Anthropic、OpenAI Compatible），用户可以选择任意 Provider 或同时使用多个。
...
| AI 多源 | 10 个 AI Provider 插件，用户自由选择 |
...
- **版本**：0.9.1
- **许可证**：私有（未开源）
- **生产代码**：591 文件，80,130 行
- **含测试代码**：764 文件，121,776 行
- **业务模块**：9 个（story、video、shot、prompt、asset、sync、character、scene、persistence）
- **子域**：43 个
- **AI Provider 插件**：10 个
- **数据库表**：28 张（14 业务表 + 5 关系表 + 6 缓存表 + 3 同步表）
- **回归防护规则**：76 条（R1-R76，7 大类）
- **i18n 键**：1634
...
没有契约，AI 只能靠猜测和全文搜索来理解模块边界，这在 9 个模块、43 个子域的规模下极易出错。
...
AI Animation Studio 采用 DDD（领域驱动设计）四层架构，依赖方向严格向内流动：
...
- **Result 类型**：`ok(value)` / `err(code, message)` 二值返回类型，替代异常抛出
- **语义化错误类**：12 个错误类，每个对应一个错误域
- **结构化错误码**：28 个错误码，每个有唯一标识和描述
...
if (result.ok) {
  ...
  errorLogger.error("Keyframe generation failed", undefined, { code: result.code });
}
```
...
modules 层包含 9 个业务模块，每个模块遵循统一的结构：
...
- **DI 容器**：30 个 token，5 个分类
- **存储**：14 个存储模块，每个对应一个或多个数据库表
- **网络**：API 客户端、熔断器、弹性 Fetch、重试引擎
- **AI Provider**：10 个视频生成插件
...
**规模**：16,502 行，4 个子域（planning、beat-editor、generation、template）

**子域详解**：

**planning（规划）**：故事的整体规划，包括故事创建、故事列表、故事元数据管理。这是 story 模块的入口子域，负责故事的生命周期管理。
...
3. **10 个 AI Provider 插件**：volcengine、kuaishou、zhipu、pixverse、seedance、google、openai-sora、minimax、anthropic、openai-compatible。每个插件实现 IVideoProvider 接口，通过 DI 容器注册。
...
**规模**：4,037 行，7 个子域（base、character、scene、beat-image、video、server-prompts、builder）
...
**规模**：1,231 行，2 个子域（services、hooks）
...
stories、story_beats、characters、scenes、video_tasks、shots、shot_elements、shot_references、assets、collections、storyboards、templates、versions、sessions

**5 张关系表**：

story_character_relations、story_scene_relations、beat_character_relations、beat_scene_relations、asset_tag_relations

**6 张缓存表**：

video_cache、image_cache、error_logs、auto_saves、import_export_queue、sync_queue

**3 张同步表**：

sync_vectors、sync_log、sync_conflicts

### 8.5 Schema 版本与迁移

- **当前版本**：CURRENT_SCHEMA_VERSION=3
- **迁移框架**：`migrations.ts` 中的 `runMigrations(db, currentVersion)`
- **事务安全**：迁移在 `db.transaction()` 中执行，失败自动回滚
- **最新迁移**：v3 添加 local_video_path 列

```typescript
// 迁移执行
const MIGRATIONS: Record<number, (db: MigrationDb) => void> = {
  3: (db) => {
    db.exec("ALTER TABLE video_tasks ADD COLUMN local_video_path TEXT");
    db.exec("ALTER TABLE video_cache ADD COLUMN local_path TEXT");
  },
};

function runMigrations(db: MigrationDb, currentVersion: number): void {
  db.transaction(() => {
    // 执行所有版本 > currentVersion 的迁移
...
**33 个结构化错误码**：每个错误码有唯一标识符和描述，例如 `API_TIMEOUT`、`RATE_LIMIT_429`、`AUTH_INVALID_KEY` 等。
...
const CATEGORY_PATTERNS: Array<{ category: ErrorCategory; patterns: RegExp[] }> = [
  { category: "timeout", patterns: [/timeout/i] },
  { category: "rate_limit", patterns: [/rate[\s_-]?limit/i] },
  // ...
];
...
AI Animation Studio 使用 Electron 的标准双进程模型：
...
主进程管理 AI Provider 插件：

- **Plugin Registry**：注册所有可用插件
- **User Plugin Loader**：加载用户自定义插件
- **Provider 实例化**：根据用户选择的 Provider 创建实例
...
| 分类 | 数量 | 描述 | 示例 |
|------|------|------|------|
| A. Domain Port 实现 | 9 | Port 接口的实现 | videoProvider, characterStorage |
| B. 有状态服务 | 6 | 需要测试替换的单例 | eventBus, apiClient |
| C. Storage 实例 | 11 | 有状态的存储模块 | versionStorage, videoCacheStorage |
...
# 运行特定模块测试
npx vitest run src/modules/story
...
if (result.ok) {
  ...
  // 处理 result.code 和 result.message
}
```
...
76 条回归防护规则（R1-R76）是从历史 Bug 中抽象出的检测规则，防止已知 Bug 模式重现。这些规则是**回归防护**，不是发现工具——下一次审计必须从零开始发现新问题。
...
ErrorBoundary 的重试次数必须有限制，避免无限重试循环。

**R19：视频 onError 使用 data-retried 守卫（与 R7 互补）**

强调使用 `data-retried` 属性而非其他方式防止重试循环。

**R20：ErrorBoundary 重试限制（与 R16 互补）**

强调所有 ErrorBoundary 实现都必须有重试限制。
...
1. **Bug 审计报告**：`docs/bug-audit-report.md`，记录所有发现的问题
...
| 类别 | 描述 | 典型错误码 |
|------|------|-----------|
| timeout | 超时 | TIMEOUT, ETIMEDOUT |
| rate_limit | 速率限制 | RATE_LIMITED, 429 |
| auth | 认证失败 | UNAUTHORIZED, 401, FORBIDDEN, 403 |
| server_error | 服务端错误 | INTERNAL_ERROR, 500, 502, 503 |
| network | 网络问题 | ECONNREFUSED, ECONNRESET, ENOTFOUND |
| validation | 数据验证失败 | VALIDATION_ERROR, INVALID_INPUT |
| not_found | 资源不存在 | NOT_FOUND, 404 |
| conflict | 冲突 | CONFLICT, 409 |
| unknown | 未知错误 | — |
...
| Electron 镜像依赖 | 低 | .npmrc 配置国内镜像，构建环境受限 | 活跃 |
| 版本锁定策略 | 低 | better-sqlite3 精确锁定，其他依赖用 ^ | 活跃 |
...
| Result 类型 | Result type | ok(value) / err(code, message) 二值返回类型 |
...
> 本文档是 AI Animation Studio 项目的唯一全方位指南，涵盖从设计哲学到实现细节的所有内容。如有与代码不一致之处，以代码为准，并更新本文档。
