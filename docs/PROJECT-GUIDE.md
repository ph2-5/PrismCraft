...
没有契约，AI 只能靠猜测和全文搜索来理解模块边界，这在 9 个模块、44 个子域的规模下极易出错。
...
97 条回归防护规则（R1-R104）是从历史 Bug 中抽象出的检测规则，防止已知 Bug 模式重现。这些规则是**回归防护**，不是发现工具——下一次审计必须从零开始发现新问题。
