...
> **版本**: 0.7.0-beta.1 | **最后更新**: 2026-05-25 | **代码规模**: 497 文件, 73,158 行
...
|------|------|------|
| Electron | 41 | 桌面应用框架 |
| Next.js | 16 (output: "export") | 前端框架 (静态导出) |
| React | 19 | UI 库 |
| Zustand | 5 | 状态管理 |
...
| 目录 | 文件数 | 代码行数 | 占比 |
|------|--------|----------|------|
| `src/modules/` | 202 | 26,250 | 35.8% |
| `src/infrastructure/` | 100 | 12,461 | 17.0% |
| `src/app/` | 19 | 11,200 | 15.3% |
| `src/shared/` | 69 | 5,853 | 8.0% |
| `src/domain/` | 29 | 2,476 | 3.4% |
| `src/` 小计 | 419 | 58,407 | 79.7% |
| `electron/src/` | 78 | 14,751 | 20.3% |
| **总计** | **497** | **73,158** | **100%** |
...
shared → domain only (绝不 modules 或 infrastructure)
domain → NOTHING (纯类型)
```
...
| `modules/` | `@/domain/*`, `@/shared/*`, `@/infrastructure/di` | `@/infrastructure/*` (除 DI), `@/modules/*/*/*` |
...
| `modules/` 直接导入 `@/infrastructure/*` | 通过 DI container 获取 |
...
- **output**: `"export"` — 无服务端功能
- **API Routes**: 通过 HTTP 代理到主进程 API Server
- **构建时处理**: API routes 和动态路由在构建时临时移除
- **路由**: 纯客户端路由, 无 SSR
...
| 函数 | 签名 | 用途 |
|------|------|------|
| `ok()` | `<T>(value: T) => Result<T>` | 创建成功结果 |
| `err()` | `<T, E>(error: E) => Result<T, E>` | 创建失败结果 |
| `fromThrowable()` | `<T>(fn: () => T) => Result<T>` | 将可能抛异常的同步函数转为 Result |
| `fromAsyncThrowable()` | `<T>(fn: () => Promise<T>) => Promise<Result<T>>` | 将可能抛异常的异步函数转为 Result |

#### 错误码注册表

28 个错误码, 分布在 12 个错误域中, 每个错误码标记 `retryable` 属性：

| 错误域 | 错误码 | retryable |
|--------|--------|-----------|
| `database` | DB_CONNECTION_FAILED, DB_QUERY_FAILED, DB_CONSTRAINT_VIOLATION, DB_BUSY | true, true, false, true |
| `validation` | VALIDATION_FAILED, INVALID_INPUT | false, false |
| `api` | API_REQUEST_FAILED, API_RESPONSE_INVALID | true, false |
| `network` | NETWORK_ERROR, NETWORK_TIMEOUT | true, true |
| `storage` | STORAGE_WRITE_FAILED, STORAGE_READ_FAILED | true, true |
| `generation` | GENERATION_FAILED, GENERATION_TIMEOUT, GENERATION_CANCELLED | true, true, false |
| `recovery` | RECOVERY_FAILED, RECOVERY_EXHAUSTED | true, false |
| `cache` | CACHE_MISS, CACHE_EXPIRED | false, false |
| `config` | CONFIG_MISSING, CONFIG_INVALID | false, false |
| `auth` | AUTH_FAILED, AUTH_EXPIRED | false, false |
| `state` | STATE_TRANSITION_INVALID | false |
| `system` | SYSTEM_ERROR, SYSTEM_BUSY | false, true |
...
| `pollVideoStatus` | `(taskId: string) => Promise<Result<VideoTask>>` | 轮询视频状态 |
| `getVideoUrl` | `(taskId: string) => Promise<Result<string>>` | 获取视频 URL |
...
| `trackChange` | `(entry: SyncChangeLogEntry) => Promise<Result<void>>` | 追踪变更 |
...
| 文件数 | 5 |
| 职责 | 用户反馈 (撤销/重做) |
| 状态 | ⚠️ DEPRECATED — 0 个消费者, v2.0 合并到 `@/shared/hooks/` |
...
|------|------|
| `useUndoAction` | 撤销操作 |
| `useUndoHistory` | 撤销历史管理 |
| `useDirtyTracker` | 脏数据追踪 |

#### 公共 API

| 导出 | 类型 | 说明 |
|------|------|------|
| `useUndoAction` | Hook | 撤销操作 Hook |
| `useUndoHistory` | Hook | 撤销历史 Hook |
| `useDirtyTracker` | Hook | 脏数据追踪 Hook |
...
| 文件数 | 3 |
| 职责 | 数据完整性 |
| 状态 | 2 个消费者, infrastructure 层导入违反 DDD 方向 |
...
| Hook | 说明 |
|------|------|
| `useStableDeps` | 稳定依赖, 防止 useEffect 闭包陷阱 |

##### services

| 服务 | 说明 |
|------|------|
| `sqlSanitizeService` | SQL 清理, 防止 SQL 注入 |

#### 公共 API

| 导出 | 类型 | 说明 |
|------|------|------|
| `useStableDeps` | Hook | 稳定依赖 Hook |
| `sqlSanitizeService` | Service | SQL 清理服务 |
...
| Hook | 说明 |
|------|------|
| `useSecurityConfig` | 安全配置管理 |

#### 公共 API

| 导出 | 类型 | 说明 |
|------|------|------|
| `useSecurityConfig` | Hook | 安全配置 Hook |

### 4.13 模块总览

| 模块 | 代码量 | 文件数 | 子域数 | 消费者数 | 状态 |
|------|--------|--------|--------|----------|------|
| Story | 9,454 | 54 | 5 | 多 | 活跃 |
| Video | 6,124 | 54 | 4 | 多 | 活跃 |
| Shot | 2,832 | 22 | 7 | 多 | 活跃 |
| Asset | 2,173 | 13 | 5 | 多 | 活跃 |
| Sync | 2,023 | 10 | 2 | 少 | 活跃 |
| Prompt | 1,784 | 18 | 6 | 多 | 活跃 |
| Character | 893 | 10 | 3 | 多 | 活跃 |
| Scene | 562 | 9 | 3 | 多 | 活跃 |
| Persistence | 246 | 4 | 2 | 1 | 评估合并 |
| Feedback | 175 | 5 | 1 | 0 | ⚠️ DEPRECATED |
| Integrity | 93 | 3 | 2 | 2 | DDD 违规 |
| Security | 88 | 2 | 1 | 0 | ⚠️ DEPRECATED |

---

## 5. Infrastructure 层详解

Infrastructure 层提供技术基础设施实现, 包括 DI 容器、AI 提供者、存储、网络、API 客户端等。模块层通过 DI 容器访问基础设施, 不直接导入。
...
| 类别 | 说明 | 示例 |
|------|------|------|
| **A. Domain Port 实现** | Port 接口的具体实现 | `videoProvider`, `imageProvider`, `textProvider`, `characterStorage`, `sceneStorage`, `storyStorage`, `videoTaskStorage`, `versionStorage`, `templateStorage`, `elementStorage`, `videoCacheStorage`, `collectionStorage`, `storyboardStorage` |
| **B. 有状态服务** | 需要单例和测试替换的服务 | `eventBus`, `apiClient`, `apiConfigFacade` |
| **C. Storage 实例** | 有状态存储模块, 模块无法直接导入 | 各种 storage 模块实例 |
| **D. Repository 实例** | Drizzle ORM 仓库 | `characterRepository`, `elementRepository`, `mediaAssetRepository` |
| **E. Infrastructure 桥接** | 纯函数因 ESLint 规则走 DI | `safeQuery`, `safeRun`, `detectVideoCodec`, `resolveImageUrl`, `getErrorMessage` |
...
4. **ESLint 桥接函数** — 因 DDD 导入规则无法直接导入的纯函数 (需注释说明原因)

> **注意**: `@/shared/*` 的纯函数 (如 `resolveImageUrl`、`getErrorMessage`) 应直接导入, **不走 DI**。
...
| 文件/目录 | 说明 |
|-----------|------|
| `sqlite-core.ts` | SQLite 核心操作 (`withRetry`, `safeQuery`, `safeRun`, WAL 模式配置) |
| `core.ts` | 基础 Storage 类 |
| `video-tasks/` | 视频任务存储 (JSON Container: config/provider/mediaRefs/tracking, parseConfig/parseProvider/parseMediaRefs/parseTracking) |
| `characters/` | 角色存储 |
| `scenes/` | 场景存储 |
| `stories/` | 故事存储 |
| `elements/` | 元素存储 |
...
| 文件 | 说明 |
|------|------|
| `api-server.ts` | localhost:API_SERVER_PORT, 连接追踪, 优雅关闭 |
| `api-gateway.ts` | API 网关 (认证, 代理, 模型适配) |
...
Shared 层提供跨切面 UI 组件、工具函数和错误处理。**Shared 层仅依赖 Domain 层**, 不依赖 Modules 或 Infrastructure。
...
App 层是 Next.js 页面和布局, 消费模块层通过 Context 提供的功能。
...
| `/story/beat/[beatId]` | Beat 详情页 | 动态路由, `generateStaticParams: [{ beatId: "_" }]` |
...
### 9.3 架构检查 (check-architecture.mjs)
...
### 9.4 模块 API 一致性检查 (check-module-api-consistency.mjs)
...
| 配置项 | 值 |
|--------|------|
| 框架 | Vitest + @testing-library/react + happy-dom/jsdom |
| 环境 | `jsdom` |
| 进程池 | `pool: "forks"` |
| 最大工作线程 | `maxWorkers: 2` |
| 覆盖率 Provider | V8 |
...
| 配置项 | 说明 |
|--------|------|
| 继承 | `eslint-config-next/core-web-vitals` + typescript |
| 废弃导入模式 | `@/types`, `@/lib`, `@/application/*`, `@/components` |
...
| shared 禁止 | `@/infrastructure/*`, `@/modules/*` |
...
| 规则 | 数量 | 说明 |
|------|------|------|
| `no-explicit-any` | 132 | 类型安全改进 |
| `no-unused-vars` | 95 | 死代码清理 |
| `no-img-element` | 33 | Electron 中可接受 |
| `no-restricted-imports` | 20 | 测试文件 DDD 违规 (warn) |
| `no-console` | 11 | 应改用 logger |
| `jsx-a11y/alt-text` | 10 | 可访问性 |
| `no-unescaped-entities` | 4 | JSX 转义 |
...
| 模块 | 消费者数 | 状态 | 计划 |
|------|----------|------|------|
| `feedback` | 0 | ⚠️ DEPRECATED | v2.0 合并到 `@/shared/hooks/` |
| `security` | 0 | ⚠️ DEPRECATED | v2.0 合并到 `@/shared/hooks/` |
| `persistence` | 1 | 评估合并 | 评估是否合并到其他模块 |
| `integrity` | 2 | DDD 违规 | infrastructure 层导入违反 DDD 方向, 需重构 |
...
**建议**: 如需多语言支持, 提取共享消息常量, 而非全面 i18n 改造。
...
|----|----------|------|
| domain | 覆盖较好 | 维持 70%+ |
| modules/services | 部分覆盖 | 补充到 70% |
| presentation | 覆盖不足 | 逐步补充 |

**阈值**: 70% (perFile), 重点补充 services 和 hooks。
...
| 配置 | 说明 |
|------|------|
| `tsconfig.json` 排除所有测试文件 | Vitest 独立处理测试类型 |
| MVP 阶段可接受 | — |
| 建议 | 考虑 `tsconfig.test.json` 用于更严格的 CI |
...
| 3 | **DI 必要性** | 仅 Port/有状态/测试替换/ESLint 桥接在容器中 |
...
4. **生成帧对**: useFramePairGenerator → container.imageProvider.generateImage(firstFrame) + generateImage(lastFrame) → 更新 beat.framePair
    ...
    → infrastructure/ai-providers/video.ts
      → model-adapter/index.ts (选择适配器)
        → 具体 Provider 适配器 (KlingAdapter/RunwayAdapter/...)
          ...
          → 响应解析 → 错误分类
            → 成功: 返回 Result.ok
            → 失败: 分类为 TIMEOUT/RATE_LIMITED/AUTH/NETWORK/SERVER
...
- 语义化错误: 12 种错误类型 + 28 个错误码，每个标记 retryable
...
- 移除脆弱的 [beatId] 目录移动: 改用 generateStaticParams 占位符
...
| DEV_SERVER_PORT | 3000 | Next.js 开发服务器 |
...
| 变量 | 用途 |
|------|------|
| BUILD_TARGET=electron | Next.js 构建目标 (触发 output: "export") |
...
│   │   ├── prompt/                # 提示词模块 (base, character, scene, video, builder, presentation)
│   │   ├── character/             # 角色模块 (hooks, services, presentation)
│   │   ├── scene/                 # 场景模块 (hooks, services, presentation)
│   │   ├── persistence/           # 持久化模块 (hooks, services)
│   │   ├── feedback/              # 反馈模块 (hooks) ⚠️ DEPRECATED
│   │   ├── integrity/             # 完整性模块 (hooks, services)
│   │   └── security/              # 安全模块 (hooks) ⚠️ DEPRECATED
│   ├── infrastructure/            # 基础设施
│   │   ├── di/                    # DI 容器
...
│   │   └── utils/                 # 工具函数 (image-url, toast-bridge, utils)
│   ├── app/                       # Next.js 页面
...
│       ├── plugins/               # 插件系统 (plugin-registry, providers/)
│       ├── security/              # 安全 (ssrf-guard/)
│       ├── logging/               # 日志 (logger, transports)
│       ├── lifecycle/             # 生命周期管理
│       └── config/                # 配置
├── scripts/                       # 工具脚本
│   ├── check-architecture.mjs     # DDD 架构检查
│   ├── check-module-api-consistency.mjs  # 模块 API 一致性检查
│   ├── smoke-test.ps1             # 冒烟测试
│   └── build-module-graph.ts      # 模块依赖图
├── build-electron.ps1             # Electron 构建脚本
├── docs/                          # 文档
│   ├── plugin-spec.schema.json    # 插件规范 Schema
│   └── plugin-specification.md    # 插件规范文档
├── package.json                   # 项目配置
