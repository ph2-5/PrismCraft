...
> **v3.2 更新**：首页改为品牌首页设计（Hero + 三大工作流入口 + 最近项目 + 快速入口），分镜编辑器改为四栏布局（分镜列表|提示词|元素绑定|预览），侧栏故事创作简化为单入口+stepper。配合 `ui-migration-plan.md`（UI 重构计划）使用。
...
```
总工期: 约122-183天（UI重构 + 5个Phase）
UI重构 (20-30天): 侧栏简化+路由重组+分镜编辑器四栏+品牌首页+素材库分类树+设置页拆分+角色场景详情面板+弹窗系统+拖拽排序
                   → 详见 ui-migration-plan.md，与 Phase 0 并行
Phase 0 (4-6天):   首页品牌重构（Hero + 三大工作流入口 + 最近项目 + 快速入口）+ CSS token统一 + 亮暗主题切换
...
Phase 2B(16-22天):  改进方案 + 四栏分镜编辑器 + 任务管理重构（利用已有 video-recovery + react-virtual + 现有 IPC，与2A并行）
...
并行机会: UI重构与 Phase 0 并行, Phase 0 与 Phase 1.1-1.4 并行, Phase 2A 与 Phase 2B 并行
...
## Phase 0：UI 视觉 Token 统一 + 品牌首页重构

> **目标**：
> 1. 消除首页硬编码颜色与内页 CSS 变量的"两张皮"问题
> 2. 首页重构为**品牌首页**（Hero区域 + 三大工作流入口卡片 + 最近项目 + 快速入口），作为应用的品牌门面和工作流起点
> 3. 新增亮/暗主题切换
>
...
**设计意图**：用户打开软件看到的是品牌首页——Hero区域建立品牌认知，三大工作流入口卡片（故事模式/分镜模式/快速生成）引导用户进入核心工作流，最近项目快速恢复工作，快速入口提供模块导航。首页不再承担仪表盘功能（引用地图、孤立资源、最近动态等已移至各自模块内）。
...
- `design-preview.html` — 品牌首页设计标准（Hero + 三大工作流 + 最近项目 + 快速入口）
...
首页重新排版为**品牌首页**，以 `design-preview.html` 为唯一标准：
...
> **v3.2 修正**：取消 v3.1 的侧栏"自由创作 vs 故事创作"分区设计。改为侧栏简化——故事创作合并为单入口+页面内stepper，首页改为品牌首页（Hero + 三大工作流 + 最近项目 + 快速入口）。详见 `ui-migration-plan.md`（UI 重构计划）。
...
- `npm run dev` → 首页展示品牌首页（Hero + 三大工作流 + 最近项目 + 快速入口）
...
> **入口**：不在 Sidebar 加独立导航项，而是通过首页的"故事创作"卡片进入——Phase 0 Task 0.3 已在品牌首页中预留了三大工作流入口卡片，点击"故事创作"卡片进入 `/story` 路由。
