# AI Animation Studio — 用户自定义插件规范

> 版本：1.3.0 | 最后更新：2026-05-24

## 概述

AI Animation Studio 支持用户通过 JSON 配置文件自定义 AI 提供商插件，无需编写代码即可接入任意兼容 OpenAI / 自定义 API 格式的 AI 服务。

插件文件放置在 `~/AI Animation Studio/Plugins/` 目录下，文件名以 `.plugin.json` 结尾。
...
**保留 ID**（不可使用）：`volcengine`、`kuaishou`、`zhipu`、`pixverse`、`seedance`、`google`、`openai-sora`、`minimax`、`openai-compatible`、`anthropic`
...
1. 将 `.plugin.json` 文件放入 `~/AI Animation Studio/Plugins/` 目录
...
1. 在 **设置 > 插件管理** 中找到目标插件
2. 点击"删除"按钮
3. 或手动删除 `~/AI Animation Studio/Plugins/` 中对应的 `.plugin.json` 文件
...
| `zhipu` | 智谱AI (GLM) | 视频 + 图片 |
| `pixverse` | Pixverse | 视频 + 图片 |
| `seedance` | Seedance | 视频 + 图片 |
| `google` | Google (Veo) | 视频 + 图片 |
| `openai-sora` | OpenAI (Sora) | 视频 + 图片 |
