...
export interface LegacyConfigFormat {
  textApiUrl?: string;
  textApiKey?: string;
  textModel?: string;
  imageApiUrl?: string;
  ...
  videoApiKey?: string;
  videoModel?: string;
  useCustomVideoApi?: boolean;
}

export async function getLegacyConfig(): Promise<LegacyConfigFormat | null> {
  try {
    ...
    const legacy: LegacyConfigFormat = {};

    const textConfig = getCapabilityConfig(config, "text");
    if (textConfig.provider && textConfig.modelId) {
      legacy.textApiUrl = textConfig.provider.baseUrl;
      legacy.textApiKey = textConfig.provider.apiKey;
      legacy.textModel = textConfig.modelId;
    }

    const imageConfig = getCapabilityConfig(config, "image");
    if (imageConfig.provider && imageConfig.modelId) {
      legacy.imageApiUrl = imageConfig.provider.baseUrl;
      legacy.imageApiKey = imageConfig.provider.apiKey;
      legacy.imageModel = imageConfig.modelId;
    }

    const videoConfig = getCapabilityConfig(config, "video");
    if (videoConfig.provider && videoConfig.modelId) {
      legacy.videoApiUrl = videoConfig.provider.baseUrl;
      legacy.videoApiKey = videoConfig.provider.apiKey;
      legacy.videoModel = videoConfig.modelId;
      legacy.useCustomVideoApi = true;
    }

    if (!legacy.textApiKey && !legacy.imageApiKey && !legacy.videoApiKey) {
      return null;
    }

    return legacy;
  } catch (error) {
    errorLogger.error("[API Config] 获取兼容配置失败:", error);
    return null;
  }
}
