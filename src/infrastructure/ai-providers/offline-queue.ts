...
const MAX_RETRIES = NETWORK_CONFIG.offlineQueue.maxRetries;
const MAX_RETRY_COUNT = 5;

function calculateRetryDelay(attempt: number, baseDelayMs = 5000): number {
...
const deduplicationCache = new Map<string, { value: string; expiresAt: number }>();
const DEDUPE_TTL_MS = 10 * 60 * 1000;

function pruneDeduplicationCache(): void {
  const now = Date.now();
...
function computeDeduplicationKey(type: string, payload: Record<string, unknown>): string {
...
function isPermanentError(errorMessage: string | null): boolean {
