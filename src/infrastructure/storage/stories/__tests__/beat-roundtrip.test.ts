...
function parseBeatRow(b: Record<string, unknown>) {
  const cameraContainer = safeParseJson(b.camera);
  const generationContainer = safeParseJson(b.generation);
  const metaContainer = safeParseJson(b.meta);

  const beat: Record<string, unknown> = {
    id: b.id,
    storyId: b.story_id,
    sequence: b.sequence,
    order: b.order_num ?? b.sequence,
    ...
    content: b.content,
    characterIds: (() => {
      const raw = b.character_ids_json;
      if (Array.isArray(raw)) return raw;
  ...
  if (
    generationContainer?.keyframeImageUrl ||
    generationContainer?.keyframePrompt
  ) {
    beat.keyframe = {
  ...
  if (b.local_video_path) beat.localVideoPath = b.local_video_path;
  if (b.local_keyframe_path) beat.localKeyframePath = b.local_keyframe_path;
  if (b.local_first_frame_path)
    beat.localFirstFramePath = b.local_first_frame_path;
  if (b.local_last_frame_path) beat.localLastFramePath = b.local_last_frame_path;
