...
export class AgentLoop {
  ...
  private async callLLMWithFallback(
    ...
    const handleChunk = (chunk: StreamChunk) => {
      ...
      if (chunk.toolCalls) {
        for (const tc of chunk.toolCalls) {
          mergeToolCall(accumulatedToolCalls, tc);
        }
      }
    ...
    try {
      ...
      if (!receivedAnyChunk) {
        const prompt = serializeMessages(llmMessages);
  ...
  private feedToolResultsBack(
    ...
    for (const tc of toolCalls) {
      ...
      const llmResult = truncateToolResult(result, maxResultTokens);
