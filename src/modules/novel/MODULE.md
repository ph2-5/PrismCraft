...
**原始小说回溯（Novel → Story 软关联）**：`NovelSourceDialog`（Story 详情页"查看原始小说"对话框，展示 novel_projects.story_id 关联的原始小说文本）、`NovelSourceDialogData`（对话框数据类型）。导入完成时不再物理删除 novel_projects 记录，改为填充 story_id 字段软关联到已创建的 Story，支持从 Story 回溯原始小说。
