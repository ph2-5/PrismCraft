...
export function useNovelPipeline({
  ...
  // 1. 状态容器
  const {
    state, setState, selectedSegmentIds, setSelectedSegmentIds,
    isProcessing, setIsProcessing, isImporting, setIsImporting,
    shots, setShots, storyStructure, setStoryStructure,
    treatment, setTreatment, shotContracts, setShotContracts,
  ...
  // 2. 派生 UI 标志
  const {
    stagesForMode, canProceed, showImportStep, showSegmentList,
    showStructureAnalysis, showPacingPlanning, showEntityReview,
    showShotBreakdown, showFinalize, isDone,
  } = usePipelineDerivedFlags({
    state, selectedSegmentIds, isProcessing, shots, onComplete,
  });
  ...
  const {
    ...
    state, setState, setSelectedSegmentIds,
  ...
  return {
    ...
    pendingRecoveryProjects, isLoadingRecovery, currentProjectId, lastSavedAt,
    handleImport, handleToggle, handleSelectAll, handleNext,
    handleConfirmCharacter, handleConfirmScene, handleEditCharacter, handleEditScene,
    handleMatchCharacter, handleEditShot, handleReorderShots, handleGeneratePrompts,
    handleFinalizeImport, handleAutoRun, setCurrentSegmentIndex,
    handleBeatsChange, handleShotContractsChange,
    handlePacingConfigChange, handleApplyPacing, handleResetPacing,
