...
// 测试辅助：构造 StoryStructure（suggestDurationByStructure 测试用）
function makeStructure(beats: NarrativeBeat[]) {
  return {
    beats,
...
describe("populateBeatPositionsAndDurations", () => {
  it("11. 根据 segmentIds 计算 position 与 duration", () => {
    ...
    expect(result[1]!.estimatedDuration).toBe(70);
  });
...
describe("suggestDurationByStructure", () => {
  it("27. climax beat 的 segment 时长缩短 20%", () => {
    const segments = [makeSegment({ id: "seg-1", estimatedDuration: 10 })];
    const structure = makeStructure([makeBeat({ type: "climax", segmentIds: ["seg-1"] })]);
    expect(suggestDurationByStructure(segments, structure).get("seg-1")).toBe(8);
  });

  it("28. setup beat 的 segment 时长延长 20%", () => {
    const segments = [makeSegment({ id: "seg-1", estimatedDuration: 10 })];
    const structure = makeStructure([makeBeat({ type: "setup", segmentIds: ["seg-1"] })]);
    expect(suggestDurationByStructure(segments, structure).get("seg-1")).toBe(12);
  });

  it("29. rising_action beat 保持原时长", () => {
    const segments = [makeSegment({ id: "seg-1", estimatedDuration: 10 })];
    const structure = makeStructure([makeBeat({ type: "rising_action", segmentIds: ["seg-1"] })]);
    expect(suggestDurationByStructure(segments, structure).get("seg-1")).toBe(10);
  });

  it("30. 不属于任何 beat 的 segment 保持原时长", () => {
    const segments = [makeSegment({ id: "seg-1", estimatedDuration: 10 })];
    const structure = makeStructure([makeBeat({ type: "setup", segmentIds: ["other-seg"] })]);
    expect(suggestDurationByStructure(segments, structure).get("seg-1")).toBe(10);
  });

  it("31. 时长建议 clamp 到 [2, 30]", () => {
    const segments1 = [makeSegment({ id: "seg-1", estimatedDuration: 1 })];
    const structure1 = makeStructure([makeBeat({ type: "climax", segmentIds: ["seg-1"] })]);
    expect(suggestDurationByStructure(segments1, structure1).get("seg-1")).toBe(2);
    const segments2 = [makeSegment({ id: "seg-2", estimatedDuration: 30 })];
    const structure2 = makeStructure([makeBeat({ type: "setup", segmentIds: ["seg-2"] })]);
    expect(suggestDurationByStructure(segments2, structure2).get("seg-2")).toBe(30);
  });

  it("32. segment 属于多个 beat 时取第一个 beat 的规则", () => {
    const segments = [makeSegment({ id: "seg-1", estimatedDuration: 10 })];
    const structure = makeStructure([
      makeBeat({ type: "setup", segmentIds: ["seg-1"] }),
      makeBeat({ type: "climax", segmentIds: ["seg-1"] }),
    ]);
    expect(suggestDurationByStructure(segments, structure).get("seg-1")).toBe(12);
  });
});
