...
> **模块顶层联动**：use-story-video.ts（本模块顶层）中的 useStoryVideo hook 和 useStableCompletedUrls 工具函数负责将视频任务状态与故事分镜关联。useStableCompletedUrls 通过 shallow 比较确保只有 Map 内容真正变化时才创建新引用，避免轮询更新触发下游 useStoryPersistence 等 useEffect。这些属于模块顶层文件，不在 storyboard 模块公共 API 导出中。
...
本模块的详细 AI 重构规范请参见：[.ai/modules/storyboard.md](../../../.ai/modules/storyboard.md)
