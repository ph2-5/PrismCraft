...
function findOptionLabel(options: { value: string; label: string }[], value: string | undefined): string | undefined {
  if (!value) return undefined;
  return options.find(o => o.value === value)?.label || value;
}

function buildShotInfoParts(beat: StoryBeat): string[] {
  const parts: string[] = [];
  const resolvedShot = resolveShotInstruction(beat);
  if (resolvedShot?.shotSize) parts.push(`景别：${findOptionLabel(SHOT_SIZE_OPTIONS, resolvedShot.shotSize)}`);
  if (resolvedShot?.cameraAngle) parts.push(`镜头角度：${findOptionLabel(CAMERA_ANGLE_OPTIONS, resolvedShot.cameraAngle)}`);
  if (resolvedShot?.cameraMovement) parts.push(`运镜：${findOptionLabel(CAMERA_MOVEMENT_OPTIONS, resolvedShot.cameraMovement)}`);
  if (beat.type) parts.push(`镜头类型：${beat.type}`);
  return parts;
}

function buildCharacterSection(characters: Character[] | undefined, beat: StoryBeat): string[] {
  if (!characters || characters.length === 0) return [];
  const charIds = getBeatCharacterIds(beat);
  const boundChars = charIds.map(id => characters.find(c => c.id === id)).filter((c): c is Character => c !== undefined);
...
function buildSceneSection(scenes: Scene[] | undefined, beat: StoryBeat): string[] {
  if (!scenes || scenes.length === 0 || !beat.sceneId) return [];
...
function buildFrameContextSection(context: PromptEditorContext, keyframeImageUrl: string | undefined, beat: StoryBeat): string[] {
  if (context !== "firstFrame" && context !== "lastFrame") return [];
  const parts: string[] = ["", `【重要】当前需要生成${context === "firstFrame" ? "首帧" : "尾帧"}的提示词。`];
  parts.push(
    context === "firstFrame"
...
function buildContextSection(request: PromptEditorRequest): string {
  const { beat, keyframeImageUrl, characters, scenes } = request;
  const parts: string[] = [];
  ...
  parts.push(...buildShotInfoParts(beat));
  parts.push(...buildCharacterSection(characters, beat));
  parts.push(...buildSceneSection(scenes, beat));
  ...
  if (beat.promptLayers) {
    if (beat.promptLayers.coreElements) parts.push(`核心元素：${beat.promptLayers.coreElements}`);
    if (beat.promptLayers.cameraAction) parts.push(`镜头动作：${beat.promptLayers.cameraAction}`);
    if (beat.promptLayers.styleAtmosphere) parts.push(`风格氛围：${beat.promptLayers.styleAtmosphere}`);
  }

  parts.push(...buildFrameContextSection(request.context, keyframeImageUrl, beat));
...
function buildKeyframeDefaultPrompt(beat: StoryBeat, characters?: Character[], scenes?: Scene[]): string {
  if (characters && scenes && characters.length > 0) {
    return generateBeatImagePrompt({
      beat, characters, scenes, isEnhanced: true,
      featureAnchoring: beat.featureAnchoring, shotInstruction: beat.shotInstruction,
    });
  }
  if (characters && scenes) {
    return generateSimpleBeatImagePrompt(beat, characters, scenes);
  }
  const parts: string[] = [beat.content || beat.description || ""];
  const resolvedShot = resolveShotInstruction(beat);
...
function buildFrameDefaultPrompt(beat: StoryBeat, characters?: Character[], scenes?: Scene[], context?: PromptEditorContext): string {
  const frameType = context === "firstFrame" ? "首帧" : "尾帧";
  if (characters && scenes && (characters.length > 0 || scenes.length > 0)) {
    const basePrompt = generateSimpleBeatImagePrompt(beat, characters, scenes, frameType);
    const extraParts: string[] = [];
    if (beat.keyframe?.prompt) extraParts.push(`风格参考：${beat.keyframe.prompt}`);
    if (beat.promptLayers?.coreElements) extraParts.push(`核心元素：${beat.promptLayers.coreElements}`);
    if (beat.promptLayers?.styleAtmosphere) extraParts.push(`风格氛围：${beat.promptLayers.styleAtmosphere}`);
    return [basePrompt, ...extraParts].filter(Boolean).join(", ");
  }
  const parts: string[] = [
    context === "firstFrame" ? "first frame of animation video" : "last frame of animation video",
    context === "firstFrame" ? "starting position, before action begins" : "ending position, after action completes",
    beat.content || beat.description || "",
  ];
  if (beat.keyframe?.prompt) parts.push(`style reference: ${beat.keyframe.prompt}`);
  parts.push("high quality", "detailed");
  return parts.filter(Boolean).join(", ");
}

function buildVideoDefaultPrompt(beat: StoryBeat): string {
  const parts: string[] = [beat.content || beat.description || ""];
  const resolvedShot = resolveShotInstruction(beat);
  if (resolvedShot?.cameraMovement) {
    const label = findOptionLabel(CAMERA_MOVEMENT_OPTIONS, resolvedShot.cameraMovement);
    parts.push(`运镜：${label}`);
  }
  if (beat.duration) parts.push(`时长：${beat.duration}秒`);
  if (beat.keyframe?.prompt) parts.push(`画面参考：${beat.keyframe.prompt}`);
  if (beat.framePair?.firstFrame?.prompt) parts.push(`首帧参考：${beat.framePair.firstFrame.prompt}`);
...
export function buildDefaultPrompt(request: PromptEditorRequest): string {
  ...
  if (context === "keyframe") return buildKeyframeDefaultPrompt(beat, characters, scenes);
  if (context === "firstFrame" || context === "lastFrame") return buildFrameDefaultPrompt(beat, characters, scenes, context);
  if (context === "video") return buildVideoDefaultPrompt(beat);
