...
export function AssetSelectorDialog({
  ...
  const { value: searchInput, setValue: setSearchInput, debouncedValue: debouncedSearch } =
  ...
  const filteredAssets = useMemo(() => {
    ...
    return imageAssets.filter((a) => a.name.toLowerCase().includes(q));
  }, [imageAssets, debouncedSearch]);
    ...
    >
      ...
      <div style={{ marginBottom: 12, position: "relative" }}>
        ...
        <input
          ...
          placeholder={t("dialog.searchAssetsPlaceholder")}
          value={searchInput}
          onChange={(e) => setSearchInput(e.target.value)}
          aria-label={t("dialog.searchAssetsPlaceholder")}
        />
      </div>
          ...
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p>
              {imageAssets.length === 0
                ? t("dialog.noImagesInLibrary")
                : t("dialog.noMatchingAssets")}
            </p>
          </div>
        )}
      </div>
