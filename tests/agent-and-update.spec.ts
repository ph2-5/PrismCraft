...
test.describe("Settings - Update Check", () => {
  test("should display version and update check button in system tab", async ({ page }) => {
    ...
    // 验证版本号显示（匹配 v<digit>.<digit>.<digit> 格式，避免版本升级后测试失败）
    // 实际版本号定义在 src/shared/constants/app-version.ts 的 APP_VERSION
    await expect(page.locator("text=/v\\d+\\.\\d+\\.\\d+/").first()).toBeVisible({ timeout: 5000 });
